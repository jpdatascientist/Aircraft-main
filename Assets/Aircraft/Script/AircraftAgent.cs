...
namespace Aircraft
{
    public class AircraftAgent : Agent
    {
        ...
        /// <summary>
        /// Collects observations used by agent to make decisions ここではvelocity=速度を受け取る
        /// UnityのBehaviourParameters,VectorObsevation
        /// </summary>
        /// <param name="sensor">The vector sensor</param>

        /// VectorSensorクラスの sensorオブジェクト
        /// エージェントの速度をローカル座標系で観測データとして提供し、学習アルゴリズムへ変換

        public override void CollectObservations(VectorSensor sensor)
        {
            /// AddObsevationメソッドを使用して観測データをsensorへ追加
            ...
            // 合計の　Obsevations = 3+3+3 = 9 これがBehaviourParametersのSpacesizeと一致する必要がある
        ...
        public override void Heuristic(in ActionBuffers actionBuffers)
        {
            Debug.LogError("Heuristic() was called on " + gameObject.name +
                "Make sure only the AircraftPlayer is set on Behaviour Type: Heuristic Only.");
        }
